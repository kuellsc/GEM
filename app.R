             ...
             pageWithSidebar(
               ...
               sidebarPanel(
                 ...
                 sliderInput(inputId = "Tl",
                             label = "Transmission loss rate in mm/h",
                             value = 5,
                             min = 0.1,
                             max = 10.0),
               ),
  ...
  output$Discharge <- renderPlot({
    ...
    while (i<=id) {
      if (pSeries[i]<=sia) {
        ...
        dSeries[i] <- 2/tdurs*((pSeries[i]-sia)^2/(pSeries[i]-sia+scn))/1000*barea*1E+6
        dLosses[i] <- casm*tlr/1000*tdurs/3600
        if(dSeries[i] >= dLosses[i]){
          dSeries[i]  <- dSeries[i]-dLosses[i]
        } else {
          dLosses[i]  <- dSeries[i]
          dSeries[i]  <- 0
        }
